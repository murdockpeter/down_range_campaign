...
**Tabletop Teaching Mode** is enabled for new battles. It adds an “At a real table” procedure to every action, a six-drill learning path, and persistent Rules Trace citations. Enable **Use physical dice** to type comma-separated face results before a command; Unity consumes them left-to-right (two results for Advantage or Disadvantage) while preserving the complete calculation trace. Standard compact play remains available by disabling Teaching Mode.

The extended resolver supports automatic Fan attacks and Fan suppression inside a 45-degree cone, Radius damage, missed explosive deviation, smoke and illumination areas, limited ammunition, focused Assist and crew-served bonuses, Main Effort, enhanced initiative, signals and EW jamming, selectable weapons, vehicle Mobility/Firepower/Control damage, focused repairs, passengers, persistent hazards, and fire extinguishing. These systems use the same save and PDF-cited trace framework, so existing campaign battles remain resumable while later missions can opt into the additional data fields.
...
The Unity header includes a persistent **Rules Trace** tab (`F2`). Every accepted or rejected command records its inputs, rules modifiers, path or LOS classifications, dice, thresholds, arithmetic, interrupt ordering, and final ruling. Applicable entries cite the exact authoritative Rules section and PDF page and provide a button that opens the packaged local PDF in Chrome using its page anchor; Edge and the default Windows handler remain fallbacks when Chrome is unavailable. Entries survive battle resume, can be read newest-first or chronologically, can be copied to the clipboard, and are included in the exported tactical result.
