...
function aarView() {
  ...
  if(done){
    const nextMission=Number(state.mission.number)===1?`<div style="margin-top:18px;padding-top:18px;border-top:1px solid var(--line)"><span class="eyebrow">NEXT OPERATION READY</span><h2>Mission 2 · Ghost Frequency</h2><p>Exploit RED's unchanged routine with close collection at the Grebņeva Relay.</p><button class="button primary" id="activateMissionTwo">Begin Mission #2</button></div>`:'';
    return `<article class="card"><div class="card-header"><h2>Mission ${state.mission.number} closed</h2><span class="sub good">ADJUDICATED</span></div><div class="card-body"><h2>${esc(state.mission.aar.outcome)}</h2><p>${esc(state.mission.aar.summary)}</p><button class="button" data-view-jump="history">Open campaign history</button>${nextMission}</div></article>`;
  }
  ...
  const generated=tactical?`Unity tactical result: ${tactical.outcome||'Mission complete'} (${tactical.scoreEarned??0}/${tactical.scoreAvailable??0} objective points). Battle ended after ${tactical.rounds} rounds${tactical.endedByDeadline?' at the mission deadline':' by player decision'}. Relay observation: ${tactical.observationTurns}/2. Alarm: ${tactical.alarm?`raised — ${tactical.alarmReason||'cause recorded in the tactical log'}`:'not raised'}. BLUE effective: ${tactical.effective}/${tactical.starting}. Terrain node: ${tactical.terrainLocationId||state.mission.locationId}.\n\n${tactical.log}`:'';
...
function bindView() {
  ...
  $('#activateMissionTwo')?.addEventListener('click',async()=>{try{const result=await window.campaignAPI.activateMissionTwo(state);state=result.state;normalizeMapData();normalizePlanningData();normalizeTacticalData();currentView='mission';document.querySelectorAll('.nav-item').forEach(el=>el.classList.toggle('active',el.dataset.view==='mission'));render();toast(result.activated?'Mission #2 activated · Ghost Frequency':'Mission #2 is already active');}catch(error){toast(error.message);}});
