...
test('battle request serializes campaign, board, units, weapons, and deterministic seed', () => {
  ...
  const request=createBattleRequest(state,{requestId:'req-1',createdAt:'2031-09-14T04:30:00Z',seed:42,mapPath:'map.png',rulesPdfPath:'C:/rules.pdf'});
  assert.equal(request.contractVersion,1);
  assert.equal(request.requestId,'req-1');
  assert.equal(request.settings.seed,42);
  assert.equal(request.settings.rulesPdfPath,'C:/rules.pdf');
...
test('battle result imports objectives and casualties once', () => {
  ...
  assert.equal(imported.state.mission.tacticalSummary.rulesTrace[0].rulePage,6);
